resource "vault_database_secrets_mount" "postgres_storage_ai_education_public" {
  count = local.deployment_configs.postgres_storage.count
  path  = "postgres-storage/ai-education/public"

  postgresql {
    name                     = "ai_education_public"
    ...
    allowed_roles            = local.postgres_storage_vault_roles.ai_education_public
...
resource "vault_database_secret_backend_role" "postgres_storage_ai_education_public_readonly" {
  for_each = local.deployment_configs.postgres_storage_ai_education_public_roles.for_each

  name        = each.key
  backend     = vault_database_secrets_mount.postgres_storage_ai_education_public.0.path
  db_name     = vault_database_secrets_mount.postgres_storage_ai_education_public.0.postgresql.0.name
